...
export default function Home() {
  ...
  const rosterContent = (
    <>
      <header className="flex flex-col gap-4 rounded-xl border border-slate-800 bg-slate-900 p-6 shadow md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-3xl font-semibold">Hockey Roster</h1>
          <p className="text-slate-300">Tipsy Pelicans</p>
        </div>
        <button
          onClick={() => void signOut()}
          className="self-start rounded border border-slate-700 px-4 py-2 text-sm font-medium text-slate-200 transition hover:bg-slate-800 md:self-auto"
        >
          Sign out
        </button>
      </header>
        ...
        <section className="space-y-4">
          ...
          {players && players.length === 0 && (
            <p className="text-slate-400">
              No players yet. Add your first player with the form.
            </p>
          )}
                  ...
                  <div>
                    ...
                    <div className="text-sm text-slate-400">
                      ...
                      {(player.isAdmin ?? false) && (
                        <span className="ml-2 rounded-full border border-amber-400/40 px-2 py-0.5 text-xs uppercase tracking-wide text-amber-300">
                          Admin
                        </span>
                      )}
                    </div>
      ...
      <section className="grid gap-8 lg:grid-cols-[2fr_3fr]">
        ...
        <section className="space-y-4">
          ...
          {games && games.length === 0 && (
            <p className="text-slate-400">
              No games scheduled. Add your first matchup.
            </p>
          )}
  ...
  const content = isLoading ? loadingContent : rosterContent;
